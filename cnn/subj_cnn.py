"""
    The purpose of this file is to train a subjectivity classifier using CNN
"""
...
from functions import count_parameters, word2id, collate_fn, init_weights, predict_subjectivity_CNN, plot_loss_accuracy
from parameters import LR_SUBJ, EPOCHS, TEST_PER, PATIENCE, ROOT_DIR_PATH, CNN_DIR_PATH, BATCH_SIZE
...
def main():
    ...
    torch.save(best_model.state_dict(), os.path.join(ROOT_DIR_PATH, CNN_DIR_PATH, "weights/cnn_subj.pt"))
    with open(os.path.join(ROOT_DIR_PATH, CNN_DIR_PATH, "weights/w2id_subj_cnn.pkl"), "wb") as tf:
        pickle.dump(w2id,tf)
    print("Model saved...")
    ...
    predict_subjectivity_CNN(X_test[100], best_model, w2id, device)

    print("Input: ", " ".join(X_test[40]))
    print("Correct Label: ", y_test[40])
    predict_subjectivity_CNN(X_test[40], best_model, w2id, device)
