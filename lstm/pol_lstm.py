"""
    The purpose of this file is to train a polarity classifier using LSTM
"""
...
from functions import count_parameters, word2id, collate_fn, init_weights, predict_polarity_LSTM, plot_loss_accuracy, remove_objective_sents_LSTM
from parameters import LR_POL, EPOCHS, TEST_PER, PATIENCE, ROOT_DIR_PATH, LSTM_DIR_PATH, BATCH_SIZE
...
def main(subj_filter):
    ...
    if subj_filter:
        ...
        subj_classifier = SentimentLSTM(num_embeddings=len(w2id_subj), bidirectional = True)
        subj_classifier.load_state_dict(torch.load(os.path.join(ROOT_DIR_PATH, LSTM_DIR_PATH, "weights/lstm_subj.pt")))
        subj_classifier.to(device)

        # filter the negative and positive reviews from objective sentences
        subj_pos = []
        for review in pos:
            filt_review = remove_objective_sents_LSTM(review, subj_classifier, w2id_subj, device)
            if filt_review:
    ...
    with open(os.path.join(ROOT_DIR_PATH, LSTM_DIR_PATH, "weights/w2id_pol_lstm.pkl"), "wb") as tf:
        pickle.dump(w2id,tf)
    print("Model saved")
